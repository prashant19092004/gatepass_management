...
//   const [user, setUser] = useState();
  ...
  async function fetchingHistory(){
    try{
      await axios.get("http://localhost:9000/admin/requests", {
        headers: {
  ...
  async function handleAccept(uid){
    ...
    try{
      await axios.post("http://localhost:9000/admin/handle_accept", requestid)
      .then((res)=> {
        console.log(res.data);
        setRequests(res.data);
      })
  ...
  async function handleRefuse(uid){
    // console.log(id);
    const requestid = {
      id : uid
    }
    try{
      await axios.post("http://localhost:9000/admin/handle_refuse", requestid)
      .then((res)=> {
        console.log(res.data);
        setRequests(res.data);
      })
  ...
  return (
    ...
    <div className="px-6 pb-8">
      <h1 className='text-3xl text-center mb-5 font-semibold'>Requests</h1>
              ...
              <header className="card-header flex flex-wrap items-center justify-between">
                ...
                <div>
                  <ul className="flex gap-4">
                    <li><button type="button" onClick={() => handleAccept(item._id)} className="px-2 py-2 bg-blue-600 text-white rounded-md">Approve</button></li>
                    <li><button type="button" onClick={() => handleRefuse(item._id)} className="px-2 py-2 bg-blue-600 text-white rounded-md">Refuse</button></li>
                  </ul>
                </div>
              </header>
